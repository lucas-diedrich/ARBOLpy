...
def TPM_log1p_Normalize(adata):
    ...
    sc.pp.log1p(adata)
    # returning adata feels unnecessary since everything scanpy does is 
    # inplace anyway
    # Interestingly though it does not print out the standard adata info 
    # you normally get when running 'adata'
    return adata
...
def silhouette_param_scan(adata, res_array, scan_n_res):
    ...
    for n,res in enumerate(res_array):
        ...
        if len(tmpadata.obs[f'leiden'].unique()) == 1:
            ...
            continue
        clusts.append(len(np.unique(tmpadata.obs['leiden'])))
        
        scores.append(
            silhouette_score(tmpadata.obsm['X_pca'], tmpadata.obs['leiden'])
        ) 
...
def PCA_Neighbors(adata, n_pcs):
    ...
    # knn gets quite big for this number of cells (~400 for 700k cells), this might 
    # slow it down unneccesarrily
    # the lung cell atlas pre-print (2.2M cells) uses 30 neighbors for the full scale, 
    # then they do sub-clustering and use 15 neighbors 
    # https://www.biorxiv.org/content/10.1101/2022.03.10.483747v1.full
...
def ARBOL(adata, tier: int = 0, cluster: int = 0,
          tree: str = '',
          parent=None,
          normalize_method: str = 'Pearson',
          max_tiers: int = 10, min_cluster_size: int = 100,
          silhoutte_subsampling_n: int = 7500,
          min_silhouette_res: float = 0.004,
          max_silhouette_res: int = 3,
          figdir: str = 'figs', h5dir: str = 'h5s',
          scan_n_res: int = 3):
    """ Wrapper function """
    ...
    if (adata.shape[0] > silhoutte_subsampling_n) & (silhoutte_subsampling_n < math.inf):
        ...
        #find optimum cluster resolution by silhouette analysis
        print('starting silhouette analysis')
        bestres, fig = silhouette_param_scan(adata, res_array=list(np.logspace(min_res_power, max_res_power, 30)), scan_n_res=scan_n_res)
